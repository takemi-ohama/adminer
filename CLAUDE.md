...
- 開発管理: `devtools/issues/`, `devtools/` （役割別分離完了）, 設定ファイル群
- ドキュメント: `devtools/docs/` （テスト・開発ガイド整備完了）

### ディレクトリ構造改善 (2025-10-01)
- **container/** → **devtools/** （開発・テスト環境統一）
- **container/tests** → **devtools/web** （役割明確化）
- **devtools/e2e** 新規作成（E2E環境分離）
...
- 対象: plugins/drivers/bigquery.php, plugins/login-bigquery.php, devtools/web/index.php等
...
- devtools/issues/i03.md #7指示による重要機能修正の完了記録
...
### 未実装機能の完全実装 (devtools/issues/i03.md)
1. **ソート、編集、作成、ダウンロード機能**: 現在未実装の全Adminer機能を実装
2. **包括的E2Eテスト**: devtools/e2eを使用した全メニュー・ボタン・リンクのテスト
3. **参照系・更新系テスト分離**:
   - 参照系: 既存データでのエラー検証・修正
   - 更新系: 新規データセット・テーブル作成による安全な更新テスト
4. **段階的実装**: 参照系完了→更新系シナリオ作成→最終検証

### パフォーマンス改善実装 (devtools/issues/i02.md)
...
### 📊 ログとレポート管理
- **実行ログ**: `devtools/e2e/test-results/` に自動保存（タイムスタンプ付き）
- **Playwrightレポート**: `devtools/e2e/playwright-report/index.html`
...
### 📖 詳細ガイド
完全なE2Eテスト手順は `devtools/docs/e2e-testing-guide.md` を参照してください。

## ディレクトリ構造変更記録 (2025-10-01更新)

### container/ → devtools/ リネーム完了
- **実施日時**: 2025年10月1日
- **変更概要**: 開発・テスト環境の統一的管理のため`container/`を`devtools/`にリネーム
- **影響範囲**:
  - ドキュメント内のパス参照を全て更新
  - Serena MCP記憶内容の更新が必要
  - 開発フロー手順の修正完了
