...
docker compose run --rm playwright-e2e sh -c "cd /app/container/e2e && npx playwright test tests/analyze-button-test.spec.js --project=chromium --reporter=line" 2>&1 | tee -a "$LOG_FILE"
TEST_EXIT_CODE=$?
