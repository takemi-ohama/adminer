...
LOG_FILE="$LOG_DIR/reference_test_$TIMESTAMP.log"

# ログディレクトリ作成
mkdir -p "$LOG_DIR"

echo "🔍 参照系E2Eテスト実行開始: $(date)" | tee "$LOG_FILE"
...
docker compose run --rm playwright-e2e reference-test.sh 2>&1 | tee -a "$LOG_FILE"

EXIT_CODE=${PIPESTATUS[0]}

if [ $EXIT_CODE -eq 0 ]; then
    echo "✅ 参照系E2Eテスト成功: $(date)" | tee -a "$LOG_FILE"
else
    echo "❌ 参照系E2Eテスト失敗: $(date)" | tee -a "$LOG_FILE"
...
echo "🎯 参照系テスト完了: $(date)" | tee -a "$LOG_FILE"